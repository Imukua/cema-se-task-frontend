      ...
      try {
        ...
        setTimeout(() => {
          ...
          if (foundClient) {
            ...
            const clientEnrollments = mockEnrollments.filter((e) => e.clientId === id)
            setEnrollments(clientEnrollments)
          }
          setLoading(false)
        }, 1000)
      } catch (error) {
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 sm:mt-0">
                    {client.programs.map((program: any) => (
                      <Badge key={program.id} className="bg-teal-100 text-teal-700 hover:bg-teal-200">
                        {program.name}
                      </Badge>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 pt-4 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">{client.programs.length}</p>
                    </div>
                  </div>
                </div>
              </div>
                                ...
                                <Card key={enrollment.id} className="border-none shadow-sm">
                                  <CardContent className="p-4">
                                    <div className="flex justify-between items-start mb-2">
                                      <h3 className="font-medium text-blue-700">{enrollment.program.name}</h3>
                                      <Badge
                                        className={
