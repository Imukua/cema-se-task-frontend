...
// Import types
import { Client, HealthProgram, Enrollment, EnrollmentUpdate, PaginatedResponse } from "@/lib/types/api";


export default function EditEnrollmentPage() { // Renamed component
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fetchedEnrollment: Enrollment = await enrollmentApi.getEnrollmentById(id);
  ...
  const handleRadioChange = useCallback((value: "active" | "completed" | "dropped") => {
    setFormData((prev) => ({
      ...prev,
      status: value,
    }));
  ...
  }, [errors.status]); // Depend on errors.status to correctly clear it
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedEnrollment: Enrollment = await enrollmentApi.updateEnrollment(id, formData);
      ...
      const program = programs.find((p) => p.id === updatedEnrollment.programId);
  ...
  if (!enrollment) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-gray-600 mb-6">The enrollment you are looking for does not exist or an error occurred.</p>
        <Link href="/enrollments">
          <Button className="bg-teal-600 hover:bg-teal-700">Return to Enrollments List</Button>
        </Link>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
         <div className="flex items-center gap-2">
              <Link href={`/enrollments/${id}`}> {/* Link back to enrollment details */}
                <Button variant="ghost" size="sm" className="text-gray-500 hover:text-gray-700">
                  <ArrowLeft className="h-4 w-4 mr-1" /> Back
                </Button>
              </Link>
              {/* Display client and program names if found */}
              <h1 className="text-2xl font-bold text-teal-700">Edit Enrollment: {client?.fullName || 'Unknown Client'} - {program?.name || 'Unknown Program'}</h1>
         </div>
      ...
      <Card className="bg-white/80 border-teal-100">
        <CardHeader>
          ...
          <CardDescription className="text-blue-600">Update the details for the enrollment</CardDescription>
        </CardHeader>
        <CardContent>
            <form id="edit-enrollment-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label className="text-blue-700">
                  Status <span className="text-red-500">*</span> {/* Status is editable */}
                </Label>
                <RadioGroup value={formData.status} onValueChange={handleRadioChange as (value: string) => void} className="flex space-x-4">
                 ...
                 {errors.status && <p className="text-sm text-red-500">{errors.status}</p>} {/* Display status error */}
              </div>
        ...
        <CardFooter className="flex justify-end space-x-4 border-t border-teal-100 pt-4">
          ...
          <Button
            ...
            disabled={isLoading || loadingData || !enrollment || !clients.length || !programs.length} // Disable if data is loading, submitting, or required data is missing
          >
            {isLoading ? (
              <>
