...
import { CalendarDays, Mail, Shield, Key, LockKeyhole, UserCircle, Clock, CheckCircle2 } from "lucide-react";
		...
		return (
			<div className="min-h-[80vh] flex flex-col items-center justify-center py-12 bg-gradient-to-br from-red-50 to-orange-50 rounded-xl">
				<div className="bg-white p-8 rounded-xl shadow-lg max-w-md w-full text-center space-y-6">
					...
					<Link href="/login">
						<Button className="bg-teal-600 hover:bg-teal-700 w-full">Go to Login</Button>
					</Link>
				</div>
			</div>
		);
		...
		return (
			<div className="min-h-[80vh] flex flex-col items-center justify-center py-12 bg-gradient-to-br from-red-50 to-orange-50 rounded-xl">
				<div className="bg-white p-8 rounded-xl shadow-lg max-w-md w-full text-center space-y-6">
					...
					<p className="text-gray-600">Unable to load your user profile details.</p>
					<Link href="/dashboard">
						<Button className="bg-teal-600 hover:bg-teal-700 w-full">Return to Dashboard</Button>
					</Link>
				</div>
			</div>
		);
				...
				<Card className="border-none shadow-md bg-white/90 backdrop-blur-sm overflow-hidden">
					<CardHeader className="pb-4 relative">
						<div className="flex flex-col md:flex-row md:items-center gap-6">
							...
							<div>
								<h2 className="text-3xl font-bold text-teal-800">{user.name}</h2>
								<div className="flex flex-wrap items-center mt-3 gap-2">
									...
									<div className="flex items-center text-gray-500">
										...
										<span>Member since {formatDate(user.createdAt)}</span> {/* Use helper */}
									</div>
								</div>
							</div>
						</div>
					...
					<CardContent className="space-y-10">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-2">
							<div className="space-y-6">
								<div className="p-5 bg-teal-50 rounded-xl transition-all hover:shadow-md">
									...
									<p className="text-lg font-medium text-teal-700 pl-8">{user.email}</p>
								</div>
								<div className="p-5 bg-blue-50 rounded-xl transition-all hover:shadow-md">
									...
									<p className="text-lg font-medium text-blue-700 pl-8">{user.role}</p>
								</div>
							</div>
							<div className="space-y-6">
								<div className="p-5 bg-purple-50 rounded-xl transition-all hover:shadow-md">
									...
									<p className="text-lg font-medium text-purple-700 pl-8">Data Not Available</p> {/* Placeholder */}
								</div>
								<div className="p-5 bg-green-50 rounded-xl transition-all hover:shadow-md">
									...
									<div className="flex items-center pl-8">
										...
										<p className="text-lg font-medium text-green-700">Active</p> {/* Assuming always active */}
									</div>
								</div>
							</div>
						</div>
						...
						<div className="pt-6 border-t border-teal-100">
							<div className="flex items-center gap-2 mb-6">
								...
								<h3 className="text-xl font-medium text-teal-700">Account Security</h3>
							</div>
							<div className="flex flex-col sm:flex-row gap-4">
								{/* Link to change password page - adjust href as needed */}
								<Link href={`users/${user.id}/edit`}> {/* Assuming a dedicated change password page for the logged-in user */}
									<Button
										variant="outline"
