...
export function ClientEnrollmentTimeline({ enrollments, clientCreatedAt }: ClientEnrollmentTimelineProps) {
        ...
        sortedEvents.map((event, index) => (
          <div key={event.id} className="relative pl-10">
            <div className="absolute left-0 flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-sm border border-gray-100">
              {event.type === "creation" ? <User className="h-4 w-4 text-blue-500" /> : getStatusIcon(event.status)}
            </div>
            <div className="space-y-1">
              ...
              {event.notes && <p className="text-xs text-gray-600 italic mt-1">"{event.notes}"</p>}
            </div>
          </div>
        ))
      ) : (
        <div className="text-center py-8 text-gray-500">No timeline events found for this client.</div>
      )}
    </div>
  )
}
