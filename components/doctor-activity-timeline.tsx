...
export function DoctorActivityTimeline({
  activities,
}: DoctorActivityTimelineProps) {
  ...
  const getActivityBadge = (type: string) => {
    switch (type) {
      ...
      case "enrollment":
        return (
          <Badge className="bg-green-100 text-green-700">Enrollment</Badge>
        );
      case "update":
    ...
    <div className="relative space-y-8 before:absolute before:inset-0 before:left-4 before:h-full before:w-0.5 before:bg-gray-100">
      {sortedActivities.length > 0 ? (
        ...
        <div className="text-center py-8 text-gray-500">
          No activity found for this doctor.
        </div>
      )}
    </div>
  );
